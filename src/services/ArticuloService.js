...
class ArticuloService {
  ...
  /**
   * Actualiza un articulo conservando los campos no enviados.
   * @param {string} codigo
   * @param {Object} cambios
   * @returns {Articulo}
   */
  actualizarParcial(codigo, cambios = {}) {
    const actual = this.obtener(codigo);
    if (!actual) {
      throw new Error(`Articulo '${codigo}' no existe`);
    }

    const siguiente = {
      codigo: actual.codigo,
      descripcion: cambios.descripcion ?? actual.descripcion,
      costo: cambios.costo ?? actual.costo,
      ganancia: cambios.ganancia ?? actual.ganancia,
      iva: cambios.iva ?? actual.iva,
      stock: actual.stock,
      stockMinimo: cambios.stockMinimo ?? actual.stockMinimo,
      marcaId: cambios.marcaId ?? actual.marcaId,
      proveedorId: cambios.proveedorId ?? actual.proveedorId,
      categoriaId: cambios.categoriaId ?? actual.categoriaId,
      imagen: cambios.imagen ?? actual.imagen,
      protegido: cambios.protegido ?? actual.protegido
    };
