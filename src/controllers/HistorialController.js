...
class HistorialController {
  ...
  renderizarTabla() {
    ...
    this.movimientosFiltrados.forEach((mov, index) => {
      ...
      row.addEventListener('dblclick', () => this.api.send('open-scanner-window', mov.codigo));
      tbody.appendChild(row);
    });
  }

  abrirSeleccionado() {
    if (this.seleccionadoIndex >= 0 && this.seleccionadoIndex < this.movimientosFiltrados.length) {
      this.api.send('open-scanner-window', this.movimientosFiltrados[this.seleccionadoIndex].codigo);
    }
  }
