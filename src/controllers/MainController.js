...
class MainController {
  constructor() {
    ...
    this.columnasVisibles = {
      codigo: true, descripcion: true, costo: true, ganancia: true,
      precioFinal: true, stock: true, stockMinimo: true, marca: true, proveedor: true
    };
  ...
  async init() {
    ...
    await this.cargarEstadoConexiones();
  }
   ...
   */
  setupEventListeners() {
    // Escuchar cambios de conexion del telefono en tiempo real
    this.api.on('phone-connection-changed', (data) => {
      // Recargar estado completo para tener la URL actualizada
  ...
  async aplicarTema() {
    ...
    if (result.success) {
      ...
      if (this.config.columnasVisibles) {
        try {
          this.columnasVisibles = JSON.parse(this.config.columnasVisibles);
  ...
  /**
   * Actualiza la visibilidad de los encabezados de la tabla
   */
  actualizarHeaders() {
    ...
    const map = ['codigo', 'descripcion', 'costo', 'ganancia', 'precioFinal', 'stock', 'stockMinimo', 'marca', 'proveedor'];
    
    headers.forEach((th, index) => {
      if (index < map.length) {
        const key = map[index];
        const visible = this.columnasVisibles[key] !== false;
  ...
  renderizarTabla() {
    ...
    this.articulosFiltrados.forEach(articulo => {
      ...
      if (v.codigo !== false) html += `<td>${articulo.codigo}</td>`;
      if (v.descripcion !== false) html += `<td>${articulo.descripcion}</td>`;
      if (v.costo !== false) html += `<td>$${costo.toFixed(2)}</td>`;
      if (v.ganancia !== false) html += `<td>${ganancia.toFixed(2)}%</td>`;
      if (v.precioFinal !== false) html += `<td>
            <div>$ ${precioFinal.toFixed(2)}</div>
            <div style="font-size: 0.85em; color: #2ecc71;">u$s ${precioUsd.toFixed(2)}</div>
        </td>`;
      if (v.stock !== false) html += `<td class="text-center">${articulo.stock}</td>`;
      if (v.stockMinimo !== false) html += `<td class="text-center">${articulo.stockMinimo}</td>`;
      if (v.marca !== false) html += `<td>${marca ? marca.nombre : '-'}</td>`;
      if (v.proveedor !== false) html += `<td>${proveedor ? proveedor.nombre : '-'}</td>`;
  ...
  calcularPrecios(articulo) {
    ...
    const ivaGlobal = (Number.isFinite(ivaGlobalRaw) ? ivaGlobalRaw : 21) / 100;
  ...
  async cargarEstadoConexiones() {
    try {
      const status = await this.api.invoke('get-connection-status');
      if (!status) return;

      // Drive
      const drivePill = document.getElementById('driveStatusPill');
      if (drivePill) {
        if (status.driveAccessible) {
          drivePill.textContent = '📁 Drive: Conectado';
          drivePill.style.background = '#1a7f37';
        } else if (status.driveConfigured) {
          drivePill.textContent = '📁 Drive: Carpeta no encontrada';
          drivePill.style.background = '#9a3412';
        } else {
          drivePill.textContent = '📁 Drive: No configurado';
          drivePill.style.background = '#555';
        }
        drivePill.title = status.drivePath || 'Sin ruta configurada (ve a Config)';
      }

      this.actualizarPillTelefono(status.phoneConnected, status.phoneUrls, status.primaryUrl, status.mdnsUrl);
    } catch (error) {
      console.error('Error al cargar estado de conexiones:', error);
    }
  }

  actualizarPillTelefono(connected, urls, primaryUrl, mdnsUrl) {
    const phonePill = document.getElementById('phoneStatusPill');
    const phoneUrlInfo = document.getElementById('phoneUrlInfo');
    const mobileIpBox = document.getElementById('mobileIpBox');
    const mobileIpText = document.getElementById('mobileIpText');
    const mobileLocalText = document.getElementById('mobileLocalText');
    const btnCopyIp = document.getElementById('btnCopyIp');

    if (phonePill) {
      if (connected) {
        phonePill.textContent = '📱 Teléfono: Conectado';
        phonePill.style.background = '#1a7f37';
      } else {
        phonePill.textContent = '📱 Teléfono: Esperando...';
        phonePill.style.background = '#555';
      }
    }

    const url = primaryUrl || (urls && urls[0]) || null;
    if (url && mobileIpBox) {
      // Dirección mDNS automática (.local)
      if (mobileLocalText) {
        if (mdnsUrl) {
          mobileLocalText.textContent = mdnsUrl.replace(/^https?:\/\//i, '');
          mobileLocalText.style.display = '';
          mobileLocalText.title = 'Dirección automática — funciona sin saber la IP';
        } else {
          mobileLocalText.style.display = 'none';
        }
      }

      // IP manual como respaldo
      if (mobileIpText) {
        const display = url.replace(/^https?:\/\//i, '');
        mobileIpText.textContent = display;
        if (urls && urls.length > 1) {
          mobileIpText.title = 'Otras IPs: ' + urls.slice(1).map(u => u.replace(/^https?:\/\//i, '')).join(', ');
        }
      }

      mobileIpBox.style.display = 'flex';

      if (btnCopyIp && !btnCopyIp._bound) {
        btnCopyIp._bound = true;
        btnCopyIp.addEventListener('click', () => {
          const copyTarget = mdnsUrl || url;
          const doCopy = () => {
          ...
          navigator.clipboard.writeText(copyTarget).catch(doCopy);
          const orig = btnCopyIp.textContent;
          btnCopyIp.textContent = '✓ Copiado';
          btnCopyIp.style.background = '#1a7f37';
          setTimeout(() => { btnCopyIp.textContent = orig; btnCopyIp.style.background = '#0078D4'; }, 1800);
        });
      }
    } else if (mobileIpBox) {
      mobileIpBox.style.display = 'none';
    }

    if (phoneUrlInfo) phoneUrlInfo.textContent = '';
  }
}
