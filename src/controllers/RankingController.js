...
class RankingController {
  ...
  async cargarRanking() {
    ...
    if (result.success) {
      ...
      this.rankingFiltrados = [...this.ranking];
  ...
  buscar(termino) {
    if (!termino) {
      ...
      this.rankingFiltrados = this.ranking.filter(a => 
  ...
  renderizarTabla() {
    const ranking = this.rankingFiltrados;
    const tbody = document.getElementById('rankingTableBody');
    tbody.innerHTML = '';
    this.seleccionadoIndex = -1;
    ...
    ranking.forEach((item, index) => {
      ...
      row.addEventListener('dblclick', () => this.api.send('open-scanner-window', item.codigo));
      tbody.appendChild(row);
    });
  }

  abrirSeleccionado() {
    if (this.seleccionadoIndex >= 0 && this.seleccionadoIndex < this.rankingFiltrados.length) {
      this.api.send('open-scanner-window', this.rankingFiltrados[this.seleccionadoIndex].codigo);
    }
  }
  ...
  navegarTabla(direccion) {
    if (this.rankingFiltrados.length === 0) return;
    let nuevoIndex = this.seleccionadoIndex + direccion;
    if (nuevoIndex < 0) nuevoIndex = 0;
    if (nuevoIndex >= this.rankingFiltrados.length) nuevoIndex = this.rankingFiltrados.length - 1;
