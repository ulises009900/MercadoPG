...
class FaltantesController {
  ...
  setupEventListeners() {
    ...
    document.addEventListener('keydown', (e) => {
      ...
      if (e.key === 'Enter') this.abrirSeleccionado();
  ...
  async cargarFaltantes() {
    ...
    if (result.success) {
      ...
      this.faltantesFiltrados = [...this.faltantes];
  ...
  buscar(termino) {
    if (!termino) {
      ...
      this.faltantesFiltrados = this.faltantes.filter(a => 
  ...
  renderizarTabla() {
    const faltantes = this.faltantesFiltrados;
    const tbody = document.getElementById('faltantesTableBody');
    tbody.innerHTML = '';
    this.seleccionadoIndex = -1;
    ...
    faltantes.forEach((art, index) => {
      ...
      row.addEventListener('dblclick', () => this.api.send('open-scanner-window', art.codigo));
      tbody.appendChild(row);
    });
  }

  abrirSeleccionado() {
    if (this.seleccionadoIndex >= 0 && this.seleccionadoIndex < this.faltantesFiltrados.length) {
      this.api.send('open-scanner-window', this.faltantesFiltrados[this.seleccionadoIndex].codigo);
    }
  }
  ...
  navegarTabla(direccion) {
    if (this.faltantesFiltrados.length === 0) return;
    let nuevoIndex = this.seleccionadoIndex + direccion;
    if (nuevoIndex < 0) nuevoIndex = 0;
    if (nuevoIndex >= this.faltantesFiltrados.length) nuevoIndex = this.faltantesFiltrados.length - 1;
